...
jQuery(function($){
	...
	$('.client-table').slick({
	  ...
	  responsive: [
		...
		/* ----------------------------------------------------------- */
	/*  5. TECHNOLOGY SLIDE (SLICK SLIDER)
	/* ----------------------------------------------------------- */

	$('.technology-table').slick({
	  dots: false,
	  infinite: true,
	  arrows:false,
	  speed: 2000,
	...
	/* ----------------------------------------------------------- */ 
		...
		// Cache selectors
		var lastId,
		topMenu = $(".header-btn-area, .main-nav"),
		...
		// WHEN CLICK KNOWMORE BUTTON
		$('.header-btn-area .knowmore-btn').click(function(){
			$("#aboutbtn").click()
		});
	...
	jQuery(window).load(function() { // makes sure the whole site is loaded
      ...
      jQuery('body').delay(100).css({'overflow-y':'visible'});
    })
	...
	/* ----------------------------------------------------------- */
	/*  13. Contact Box Sizing
	/* ----------------------------------------------------------- */
	$(document).ready(function(){
		setTimeout(function(){
			var addressBoxHeight = $('.contact-left').outerHeight();
			$('.contact-right').css('height', addressBoxHeight);
		}, 300);
		//var addressBoxHeight = $('.contact-left').outerHeight();
		//$('.contact-right').css('height', addressBoxHeight);
	});
	...
	/* ----------------------------------------------------------- */
	/*  14. URL Changer for search
	/* ----------------------------------------------------------- */
	$('form.blog-search').submit(function(e){
	e.preventDefault();
	var search_field = $(this).find('#search_query');
	var search_query = search_field.val().trim();
	if(search_query) {
        window.location.href = "?search=" + search_query;
	}
	else {
	    search_field.val('');
	    search_field.css({'outline': '0', 'border-color': '#ed9e9e', 'box-shadow': '0 0 10px #ed9e9e'});
	    search_field.attr('placeholder', 'Required');
	}
	});
